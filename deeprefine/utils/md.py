...
def run_md(startPDB, prefix,
    ...
    integrator = LangevinIntegrator(
        temperature*unit.kelvin, friction/unit.picosecond, stepsize*unit.picoseconds)
    simulation = Simulation(modeller.topology, system, integrator)
    platform = simulation.context.getPlatform()
    if verbose: print(f"Simulation running with {platform.getName()}", flush=True)
    simulation.context.setPositions(modeller.positions)
    if verbose: print("Minimizing... ", end="", flush=True)
    ...
    if verbose:
        ...
        statereporter = StateDataReporter(stdout, int(1000*reportstep/stepsize), step=True, time=True,
                                        volume=True, totalEnergy=True, temperature=True, elapsedTime=True)
    else:
        statereporter = StateDataReporter(prefix+'_equi_report.csv', int(1000*reportstep/stepsize), step=True, time=True,
                                        volume=True, totalEnergy=True, temperature=True, elapsedTime=True) 
    simulation.reporters.append(statereporter)
    simulation.step(int(1000*eqtime/stepsize))
    if verbose: print("Equilibrating... done", flush=True)
    ...
    if verbose: print("Production run... ", end="", flush=True)
    # simulation.loadCheckpoint("implicit_equilibrated.chkpt")
    simulation.context.setVelocitiesToTemperature(temperature*unit.kelvin)
    statereporter = StateDataReporter(prefix+"_report.csv", int(1000*reportstep/stepsize), step=True,
                                      time=True, volume=True, totalEnergy=True, temperature=True, elapsedTime=True)
    ...
    trajreporter.close()
    if verbose: print("MD running done", flush=True)