...
def SO3_to_quaternions(mats) -> np.array:
    ...
    signs = np.sign(
        np.einsum("nx,nx->n", np.cross(vs, np.einsum("nxy,ny->nx", mats, vs)), us)
    )
...
def getbestneighbours_cartesian(
    loss,
    current_uvw_array_frac,
    basegrid=24.0,
    ...
    scale = (1.0 / (basegrid - 1.0)) / 2 ** (curr_res)
    ...
    duvw_array_frac = scale * np.array(
        np.meshgrid(du_list, dv_list, dw_list)
    ).T.reshape(-1, 3)
