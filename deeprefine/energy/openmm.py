    ...
    class Worker(mp.Process):
        ...
        def run(self):
            ...
            for task in iter(self._task_queue.get, None):
                ...
                try:
                    # initialize state
                    self._openmm_context.setPositions(
                        unit.Quantity(value=positions, unit=self._length_scale)
                    )
                    ...
                    forces = (
                        state.getForces(asNumpy=True).value_in_unit(
                            unit.kilojoule_per_mole / self._length_scale
                        )
                        if evaluate_force
                        else None
                    )
                    new_positions = (
                        state.getPositions().value_in_unit(self._length_scale)
                        if evaluate_positions
                        else None
                    )
                except Exception as e:
    ...
    def evaluate(
        ...
        for i, (p, bv) in enumerate(zip(positions, box_vectors)):
            ...
            try:
                # initialize state
                self._openmm_context.setPositions(
                    unit.Quantity(value=p, unit=self._length_scale)
                )
                if bv is not None:
...
class OpenMMEnergy(_BridgeEnergy):
    ...
    def energy(self, xs, temperature=1.0, **kwargs):
        ...
        """
        assert (
            xs.shape[1] == self.event_shape[0]
        ), f"Expected {self.event_shape[0]} dimensionality but received {xs.shape[1]}"
