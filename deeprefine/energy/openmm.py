'''
Wrap openmm potential energy calculation as torch function with grads
I modified the codes from https://github.com/noegroup/bgflow/blob/main/bgflow/distribution/energy/openmm.py
They provided a robust implementation to run openmm on different platforms

1. Add an extra argument `length scale` as openmm default is nanometer but PDB uses angstroms
2. Provide a utility function `setup_protein` to generate a openmmEnergy object from pdb file.
'''
...
from deeprefine.energy.base import _BridgeEnergy, _Bridge
from deeprefine.utils.types import assert_numpy

from openmm import unit

def setup_protein(pdbmodel_dir, temp, implicit_solvent=True, length_scale=unit.angstroms, platform='CPU', n_simulation_steps=0):
    ...
    openmm_bridge = OpenMMBridge(system, integrator, length_scale, platform_name=platform, n_simulation_steps=n_simulation_steps)
    openmm_energy = OpenMMEnergy(openmm_bridge)

    mdtraj_topology = md.Topology().from_openmm(pdb.topology)
    return mdtraj_topology, openmm_energy
...
class OpenMMBridge(_Bridge):
    ...
    length_scale : detault openmm.unit.nanometers
        length unit of your model coordinates, in openmm.unit
    platform_name : str, optional
        An OpenMM platform name ('CPU', 'CUDA', 'Reference', or 'OpenCL')
    ...
    def __init__(
        ...
        if platform_name in ["CUDA", "OpenCL"] or n_workers == 1:
            self.context_wrapper = SingleContext(
                1, openmm_system, openmm_integrator, length_scale, platform_name, platform_properties
            )
        else:
            self.context_wrapper = MultiContext(
                n_workers, openmm_system, openmm_integrator, length_scale, platform_name, platform_properties
            )
    ...
    def evaluate(
        ...
        batch : np.ndarray or torch.Tensor
            A batch of particle positions that has shape (batch_size, num_particles * 3), in the unit of self._length_scale
...
class MultiContext:
    ...
    def __init__(self, n_workers, system, integrator, length_scale, platform_name, platform_properties={}):
    ...
    class Worker(mp.Process):
        ...
        def __init__(self, task_queue, result_queue, system, integrator, length_scale, platform_name, platform_properties):
        ...
        def run(self):
            ...
            Positions and box vectors are received from the task_queue in units of self._length_scale.
            ...
            for task in iter(self._task_queue.get, None):
                ...
                try:
                    # initialize state
                    self._openmm_context.setPositions(unit.Quantity(value=positions, unit=self._length_scale))
                    ...
                    forces = (
                        state.getForces(asNumpy=True).value_in_unit(unit.kilojoule_per_mole / self._length_scale)
                        if evaluate_force else None
                    )
                    new_positions = state.getPositions().value_in_unit(self._length_scale) if evaluate_positions else None
                except Exception as e:
...
class SingleContext:
    ...
    def evaluate(
        ...
        for i, (p, bv) in enumerate(zip(positions, box_vectors)):
            ...
            try:
                # initialize state
                self._openmm_context.setPositions(unit.Quantity(value=p, unit=self._length_scale))
                if bv is not None:
                ...
                new_pos = state.getPositions().value_in_unit(self._length_scale) if evaluate_positions else None
...
class OpenMMEnergy(_BridgeEnergy):
    def __init__(self, bridge=None, two_event_dims=False):
        self.length_scale = bridge._length_scale
        super().__init__(bridge, two_event_dims=two_event_dims)

    def energy(self, xs, temperature=1.0, **kwargs):
        '''
        xs: torch.tensor, [B, N*D]
            model coordinates in unit of self.length_scale
        '''
        assert xs.shape[1] == self.event_shape[0], f"Expected {self.event_shape[0]} dimensionality but received {xs.shape[1]}"
